  ...
  def Back_Prop(self,eta,classNumber,actual,output_size):  
    ...
    for i in reversed(range(len(self.mlp_init))):   #go through every layer backwards
      ...
      if i == len(self.mlp_init)- 1:    #output layer
        ...
        #print('hello :)')
        if classNumber == 1:
          diff = actual - self.optput              #delta is actual - predicted * derivative of the actication function. So for the sigmoid layers this would be (ri-yi)(oj(1-oj)) and linear it would just be (ri-yi) * possibly C
          print(diff)
